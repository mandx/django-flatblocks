...
class FlatBlockDefaultWrapper(object):
    def __init__(self, end_tag_name):
        self.end_tag_name = end_tag_name
        super(FlatBlockDefaultWrapper, self).__init__()

    def prepare(self, parser, token):
        """
        The parser checks for following tag-configurations::

            {% flatblock_default {block} %}{% end_flatblock_default %}
            {% flatblock_default {block} {timeout} %}{% end_flatblock_default %}
    ...
    def __call__(self, parser, token):
        self.prepare(parser, token)
        return FlatBlockDefaultNode(self.slug, self.is_variable,
                self.inner_nodelist, self.cache_time,
                template_name=self.tpl_name,
                tpl_is_variable=self.tpl_is_variable)

class FlatBlockDefaultNode(template.Node):
    def __init__(self, slug, is_variable, inner_nodelist, cache_time=0,
                 with_template=True, template_name=None, tpl_is_variable=False):
        if template_name is None:
            self.template_name = 'flatblocks/flatblock.html'
        else:
            if tpl_is_variable:
        ...
        try:
            ...
            if flatblock is None:
                ...
                if self.is_variable or not settings.AUTOCREATE_STATIC_BLOCKS:
                    ...
                    flatblock, _ = FlatBlock.objects.get_or_create(
                        slug=real_slug,
                        defaults = { 'content': self.inner_nodelist.render(context) })
...
class PlainFlatBlockDefaultWrapper(FlatBlockDefaultWrapper):
    def __call__(self, parser, token):
        self.prepare(parser, token)
        return FlatBlockDefaultNode(self.slug, self.is_variable, self.inner_nodelist, self.cache_time, False)

do_get_flatblock = FlatBlockDefaultWrapper('end_flatblock_default')
do_plain_flatblock = PlainFlatBlockDefaultWrapper('end_plain_flatblock_default')

register.tag('flatblock_default', do_get_flatblock)
register.tag('plain_flatblock_default', do_plain_flatblock)
